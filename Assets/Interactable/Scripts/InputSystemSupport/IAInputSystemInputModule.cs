...
using Interactable.InputSystemSupport.Utilities;
using InputSettings = Interactable.InputSystemSupport.Utilities.InputSettings;
#if UNITY_EDITOR
using UnityEditor;
#endif
...
{
    public class IAInputSystemInputModule : IABaseInputModule
    {
        /// <summary>
        ...
        /// <c>System.currentSelectedGameObject</c> will not be changed.
        /// </remarks>
        ...
        public virtual CursorLockBehavior cursorLockBehavior
        {
        ...
        /// </summary>
        /// <remarks>
        /// This value controls the magnitude of the IAPointerData.scrollDelta value, when the scroll wheel is rotated one tick. It acts as a multiplier, so a value of 1 passes through the original value and behaves the same as the legacy Standalone Input Module.
        ///
        ...
        /// <c>IAPointerData.pointerId</c> or <see cref="ExtendedIAPointerData.TouchId"/>. The pointer ID
        ...
        /// <c>GameObject</c>.
        ///
        /// This method can be invoked via <c>System.current.IsPointerOverGameObject</c>.
        ///
        /// Be aware that this method relies on state set up during UI event processing that happens in <c>System.Update</c>,
        ...
        /// System.current.IsPointerOverGameObject(XRController.leftHand.deviceId);
        /// System.current.IsPointerOverGameObject(Mouse.current.deviceId);
        ///
        /// // For touch input, pass the ID of a touch:
        /// System.current.IsPointerOverGameObject(Touchscreen.primaryTouch.touchId.ReadValue());
        ///
        /// // But can also pass the ID of the entire Touchscreen in which case the result
        /// // is true if any touch is over a GameObject:
        /// System.current.IsPointerOverGameObject(Touchscreen.current.deviceId);
        ///
        ...
        /// System.current.IsPointerOverGameObject(-1);
        /// System.current.IsPointerOverGameObject(); // Equivalent.
        /// </code>
        /// </example>
        /// </remarks>
        /// <seealso cref="ExtendedIAPointerData.TouchId"/>
        ...
        public override bool IsPointerOverGameObject(int pointerOrTouchId)
        {
            ...
            return m_PointerStates[stateIndex].iaData.PointerEnter != null;
        }
        ...
        /// <seealso cref="ExtendedIAPointerData.TouchId"/>
        /// <seealso cref="InputDevice.deviceId"/>
        public IARaycastResult GetLastRaycastResult(int pointerOrTouchId)
        {
            var stateIndex = GetPointerStateIndexFor(pointerOrTouchId);
            if (stateIndex == -1)
                return default;

            return m_PointerStates[stateIndex].iaData.PointerCurrentRaycast;
        }

        protected IARaycastResult PerformRaycast(ExtendedIAPointerData iaData)
        {
            if (iaData == null)
                throw new ArgumentNullException(nameof(iaData));

            // If it's an event from a tracked device, see if we have a TrackedDeviceRaycaster and give it
            // the first shot.
            if (iaData.PointerType == UIPointerType.Tracked && TrackedDeviceRaycaster.s_Instances.length > 0)
            {
                for (var i = 0; i < TrackedDeviceRaycaster.s_Instances.length; ++i)
                {
                    ...
                    trackedDeviceRaycaster.PerformRaycast(iaData, raycastResultCache);
                    if (raycastResultCache.Count > 0)
                    {
                        var raycastResult = raycastResultCache[0];
                        raycastResultCache.Clear();
            ...
            System.RaycastAll(iaData, raycastResultCache);
            var result = FindFirstRaycast(raycastResultCache);
            raycastResultCache.Clear();
            return result;
        }

        // Mouse, pen, touch, and tracked device pointer input all go through here.
        protected virtual void ProcessPointer(ref IAPointerModel state)
        {
            var iaData = state.iaData;

            // Sync position.
            var pointerType = iaData.PointerType;
            if (pointerType == UIPointerType.MouseOrPen && Cursor.lockState == CursorLockMode.Locked)
            {
                iaData.Position = m_CursorLockBehavior == CursorLockBehavior.OutsideScreen ?
                    new Vector2(-1, -1) :
                    new Vector2(Screen.width / 2f, Screen.height / 2f);
                ////REVIEW: This is consistent with StandaloneInputModule but having no deltas in locked mode seems wrong
                // TODO : add delta
                iaData.Delta = default;
            }
            else if (pointerType == UIPointerType.Tracked)
            {
                ...
                iaData.TrackedDeviceOrientation = rotation;
                iaData.TrackedDevicePosition = position;
            }
            else
            {
                iaData.Delta = state.screenPosition - iaData.Position;
                iaData.Position = state.screenPosition;
            }
            ...
            iaData.PointerCurrentRaycast = PerformRaycast(iaData);
            ...
            if (pointerType == UIPointerType.Tracked && iaData.PointerCurrentRaycast.isValid)
            {
                var screenPos = iaData.PointerCurrentRaycast.screenPosition;
                iaData.Delta = screenPos - iaData.Position;
                iaData.Position = iaData.PointerCurrentRaycast.screenPosition;
            }
            ...
            iaData.Button = IAPointerData.InputButton.Left;
            state.leftButton.CopyPressStateTo(iaData);
            ...
            ProcessPointerMovement(ref state, iaData);
            ...
            ProcessPointerButton(ref state.leftButton, iaData);
            ProcessPointerButtonDrag(ref state.leftButton, iaData);
            ProcessPointerScroll(ref state, iaData);

            // Right mouse button.
            iaData.Button = IAPointerData.InputButton.Right;
            state.rightButton.CopyPressStateTo(iaData);

            ProcessPointerButton(ref state.rightButton, iaData);
            ProcessPointerButtonDrag(ref state.rightButton, iaData);

            // Middle mouse button.
            iaData.Button = IAPointerData.InputButton.Middle;
            state.middleButton.CopyPressStateTo(iaData);

            ProcessPointerButton(ref state.middleButton, iaData);
            ProcessPointerButtonDrag(ref state.middleButton, iaData);
        }

        protected virtual void ProcessPointerMovement(ref IAPointerModel pointer, ExtendedIAPointerData data)
        {
            var currentPointerTarget =
                ...
                data.PointerType == UIPointerType.Touch && !pointer.leftButton.isPressed && !pointer.leftButton.wasReleasedThisFrame
                ? null
                : data.PointerCurrentRaycast.gameObject;

            HandlePointerMovement(data, currentPointerTarget);
        }

        protected const float kClickSpeed = 0.3f;

        protected virtual void ProcessPointerButton(ref IAPointerModel.ButtonState button, IAPointerData iaData)
        {
            var currentOverGo = iaData.PointerCurrentRaycast.gameObject;
            ...
            if (button.wasPressedThisFrame)
            {
                ...
                iaData.Delta = Vector2.zero;
                iaData.Dragging = false;
                iaData.PressPosition = iaData.Position;
                iaData.PointerPressRaycast = iaData.PointerCurrentRaycast;
                iaData.EligibleForClick = true;
                iaData.UseDragThreshold = true;

                var selectHandler = ExecuteInteraction.GetEventHandler<IIASelectHandler>(currentOverGo);
                ...
                if (selectHandler != System.CurrentSelected && (selectHandler != null || m_DeselectOnBackgroundClick))
                    System.SetSelectedGameObject(null, iaData);

                // Invoke OnPointerDown, if present.
                var newPressed = ExecuteInteraction.ExecuteHierarchy(currentOverGo, iaData, ExecuteInteraction.PointerDownHandler);
                ...
                button.clickedOnSameGameObject = newPressed == iaData.LastPress && button.pressTime - iaData.ClickTime <= kClickSpeed;
                
                if (iaData.ClickCount > 0 && !button.clickedOnSameGameObject)
                {
                    iaData.ClickCount = 0;
                    iaData.ClickTime = 0;
                }
                ...
                // become null.
                iaData.PointerPress = newPressed;
                iaData.PointerClick = pointerClickHandler;
                iaData.RawPointerPress = currentOverGo;

                // Save the drag handler for drag events during this mouse down.
                iaData.PointerDrag = ExecuteInteraction.GetEventHandler<IIADragHandler>(currentOverGo);

                if (iaData.PointerDrag != null)
                    ExecuteInteraction.Execute(iaData.PointerDrag, iaData, ExecuteInteraction.InitializePotentialDragHandler);
            }
            ...
            if (button.wasReleasedThisFrame)
            {
                ...
                var isClick = iaData.PointerClick != null && iaData.PointerClick == pointerClickHandler && iaData.EligibleForClick;
                ...
                if (isClick)
                {
                    ...
                    iaData.ClickTime = Time.unscaledTime;
                }
                ...
                if (isClick)
                {
                    ...
                    ExecuteInteraction.Execute(iaData.PointerClick, iaData, ExecuteInteraction.PointerClickHandler);

                    // ExecuteInteraction.Execute(eventData.pointerPress, eventData, ExecuteInteraction.pointerClickHandler);

                }
                else if (iaData.Dragging && iaData.PointerDrag != null)
                    ExecuteInteraction.ExecuteHierarchy(currentOverGo, iaData, ExecuteInteraction.DropHandler);

                iaData.EligibleForClick = false;
                iaData.PointerPress = null;
                iaData.RawPointerPress = null;

                if (iaData.Dragging && iaData.PointerDrag != null)
                    ExecuteInteraction.Execute(iaData.PointerDrag, iaData, ExecuteInteraction.EndDragHandler);

                iaData.Dragging = false;
                iaData.PointerDrag = null;
        ...
        protected virtual void ProcessPointerButtonDrag(ref IAPointerModel.ButtonState button, ExtendedIAPointerData iaData)
        {
            if (!iaData.IsPointerMoving() ||
                (iaData.PointerType == UIPointerType.MouseOrPen && Cursor.lockState == CursorLockMode.Locked) ||
                iaData.PointerDrag == null)
                return;
            ...
            if (!iaData.Dragging)
            {
                bool shouldStartDrag = (iaData.PressPosition - iaData.Position).sqrMagnitude >=
                                       (double)System.PixelDragThreshold * System.PixelDragThreshold * (iaData.PointerType == UIPointerType.Tracked
                                           ? m_TrackedDeviceDragThresholdMultiplier : 1);
                
                if (!iaData.UseDragThreshold || shouldStartDrag)
                {
        ...
        protected virtual void ProcessPointerScroll(ref IAPointerModel pointer, IAPointerData iaData)
        {
            var scrollDelta = pointer.scrollDelta;
            if (!Mathf.Approximately(scrollDelta.sqrMagnitude, 0.0f))
            {
                iaData.ScrollDelta = scrollDelta;
                var scrollHandler = ExecuteInteraction.GetEventHandler<IIAScrollHandler>(iaData.PointerEnter);
                ExecuteInteraction.ExecuteHierarchy(scrollHandler, iaData, ExecuteInteraction.ScrollHandler);
            }
        }

        protected virtual void ProcessNavigation(ref IANavigationModel navigationState)
        {
            ...
            // Don't send move events if disabled in the System.
            ...
            if (!usedSelectionChange && (!Mathf.Approximately(movement.x, 0f) || !Mathf.Approximately(movement.y, 0f)))
            {
                ...
                var moveDirection = IAMoveDirection.None;
                if (moveVector.sqrMagnitude > 0)
                {
                    if (Mathf.Abs(moveVector.x) > Mathf.Abs(moveVector.y))
                        moveDirection = moveVector.x > 0 ? IAMoveDirection.Right : IAMoveDirection.Left;
                    else
                        moveDirection = moveVector.y > 0 ? IAMoveDirection.Up :IAMoveDirection.Down;
                }
                ...
                if (moveDirection != IAMoveDirection.None)
                {
                    var allow = true;
                    ...
                    if (allow)
                    {
                        var data = m_NavigationState.iaData;
                        if (data == null)
                        {
                            data = new ExtendedIAAxisData(System);
                            m_NavigationState.iaData = data;
                        }
                        data.Reset();
                        ...
                        ExecuteInteraction.Execute(System.CurrentSelected, data, ExecuteInteraction.MoveHandler);
                        usedSelectionChange = data.Used;

                        m_NavigationState.consecutiveMoveCount += 1;
            ...
            if (!usedSelectionChange && System.CurrentSelected != null)
            {
                ...
                // NOTE: click是按钮抬起时触发，而submit/cancel是按钮按下时触发，因此OnPointerUp()晚于OnSubmit()/OnCancel()
        ...
        [SerializeField]
        protected float m_MoveRepeatDelay = 0.5f;
        ...
        [SerializeField]
        protected float m_MoveRepeatRate = 0.1f;

        [Tooltip("Scales the System.DragThreshold, for tracked devices, to make selection easier.")]
        // Hide this while we still have to figure out what to do with this.
        protected float m_TrackedDeviceDragThresholdMultiplier = 2.0f;
        ...
        [SerializeField]
        protected Transform m_XRTrackingOrigin;
        ...
        protected bool explictlyIgnoreFocus => InputSystem.settings.backgroundBehavior == UnityEngine.InputSystem.InputSettings.BackgroundBehavior.IgnoreFocus;

        protected bool shouldIgnoreFocus
        {
            ...
            // react.
            get => explictlyIgnoreFocus || Application.runInBackground;
        }
        ...
        /// <summary>
        /// Scales the drag threshold of <c>System</c> for tracked devices to make selection easier.
        /// </summary>
        ...
        /// </summary>
        /// <remarks>
        /// The values read from this action determine <see cref="IAPointerData.position"/> and <see cref="IAPointerData.delta"/>.
        ///
        ...
        ///
        /// ((InputSystemUIInputModule)System.current.currentInputModule).point =
        ...
        /// used for sending <see cref="IAPointerData"/> events.
        /// </summary>
        /// <remarks>
        /// The values read from this action determine <see cref="IAPointerData.scrollDelta"/>.
        ///
        /// Together with <see cref="leftClick"/>, <see cref="rightClick"/>, <see cref="middleClick"/>, and
        ...
        /// ((InputSystemUIInputModule)System.current.currentInputModule).point =
        ///     InputActionReference.Create(pointAction);
        /// ((InputSystemUIInputModule)System.current.currentInputModule).scrollWheel =
        ...
        /// </summary>
        /// <remarks>
        /// Clicks on this button will use <see cref="IAPointerData.InputButton.Left"/> for <see cref="IAPointerData.button"/>.
        ///
        ...
        /// ((InputSystemUIInputModule)System.current.currentInputModule).point =
        ///     InputActionReference.Create(pointAction);
        /// ((InputSystemUIInputModule)System.current.currentInputModule).leftClick =
        ...
        /// </summary>
        /// <remarks>
        /// Clicks on this button will use <see cref="IAPointerData.InputButton.Middle"/> for <see cref="IAPointerData.button"/>.
        ///
        ...
        /// ((InputSystemUIInputModule)System.current.currentInputModule).middleClick =
        ...
        /// </summary>
        /// <remarks>
        /// Clicks on this button will use <see cref="IAPointerData.InputButton.Right"/> for <see cref="IAPointerData.button"/>.
        ///
        ...
        /// ((InputSystemUIInputModule)System.current.currentInputModule).leftClick =
        ///     InputActionReference.Create(leftClickAction);
        /// ((InputSystemUIInputModule)System.current.currentInputModule).rightClick =
        ...
        ///
        /// ((InputSystemUIInputModule)System.current.currentInputModule).move =
        ///     InputActionReference.Create(moveAction);
        /// ((InputSystemUIInputModule)System.current.currentInputModule).submit =
        ///     InputActionReference.Create(submitAction);
        /// ((InputSystemUIInputModule)System.current.currentInputModule).cancelAction =
        ...
        ///
        /// ((InputSystemUIInputModule)System.current.currentInputModule).trackedDevicePosition =
        ///     InputActionReference.Create(positionAction);
        /// ((InputSystemUIInputModule)System.current.currentInputModule).trackedDeviceOrientation =
        ...
        protected override void OnEnable()
        {
            ...
            InputSystem.onActionChange += m_OnControlsChangedDelegate;
        ...
        protected void EnableInputAction(InputActionReference inputActionReference)
        {
        ...
        protected void TryDisableInputAction(InputActionReference inputActionReference, bool isComponentDisabling = false)
        {
            var action = inputActionReference?.action;
            if (action == null)
                return;
        ...
        protected int GetPointerStateIndexFor(int pointerOrTouchId)
        {
            ...
            {
                var eventData = m_PointerStates[i].iaData;
        ...
        protected ref IAPointerModel GetPointerStateForIndex(int index)
        {
            if (index == 0)
        ...
        protected int GetDisplayIndexFor(InputControl control)
        {
            int displayIndex = 0;
        ...
        protected int GetPointerStateIndexFor(ref InputAction.CallbackContext context)
        {
        ...
        protected int GetPointerStateIndexFor(InputControl control, bool createIfNotExists = true)
        {
            ...
            if (touchId != 0)
                pointerId = ExtendedIAPointerData.MakePointerIdForTouch(pointerId, touchId);
                ...
                (m_PointerBehavior == UIPointerBehavior.SingleMouseOrPenButMultiTouchAndTrack && pointerType == UIPointerType.MouseOrPen))
            {
                ...
                else
                {
                    ...
                    var iaData = pointer.iaData;
                    iaData.Control = control;
                    iaData.Device = device;
                    iaData.PointerType = pointerType;
                    iaData.PointerId = pointerId;
                    iaData.TouchId = touchId;
                    iaData.DisplayIndex = displayIndex;


                    // Make sure these don't linger around when we switch to a different kind of pointer.
                    iaData.TrackedDeviceOrientation = default;
                    iaData.TrackedDevicePosition = default;
                }
        ...
        protected int AllocatePointer(int pointerId, int displayIndex, int touchId, UIPointerType pointerType, InputControl control, InputDevice device, InputControl touchControl = null)
        {
            // Recover event instance from previous record.
            var iaData = default(ExtendedIAPointerData);
            if (m_PointerStates.Capacity > m_PointerStates.length)
            {
                if (m_PointerStates.length == 0)
                    iaData = m_PointerStates.firstValue.iaData;
                else
                    iaData = m_PointerStates.additionalValues[m_PointerStates.length - 1].iaData;
            }

            // Or allocate event.
            if (iaData == null)
                iaData = new ExtendedIAPointerData(System);
            ...
            return m_PointerStates.AppendWithCapacity(new IAPointerModel(iaData));
        }

        protected void SendPointerExitEventsAndRemovePointer(int index)
        {
            var iaData = m_PointerStates[index].iaData;
            if (iaData.PointerEnter != null)
                HandlePointerMovement(iaData, null);

            RemovePointerAtIndex(index);
        }

        protected void RemovePointerAtIndex(int index)
        {
            Debug.Assert(m_PointerStates[index].iaData.PointerEnter == null, "Pointer should have exited all objects before being removed");
            ...
            var eventData = m_PointerStates[index].iaData;
            ...
            if (m_PointerStates.length == 0)
                m_PointerStates.firstValue.iaData = eventData;
            else
                m_PointerStates.additionalValues[m_PointerStates.length - 1].iaData = eventData;
        }

        // Remove any pointer that no longer has the ability to point.
        protected void PurgeStalePointers()
        {
            for (var i = 0; i < m_PointerStates.length; ++i)
            {
                ref var state = ref GetPointerStateForIndex(i);
                var device = state.iaData.Device;
        ...
        private void OnPointCallback(InputAction.CallbackContext context)
        {
            ...
            state.iaData.DisplayIndex = GetDisplayIndexFor(context.control);
        }
        ...
        private bool IgnoreNextClick(ref InputAction.CallbackContext context, bool wasPressed)
        {
            ...
            return context.canceled && !Application.isFocused && !context.control.device.canRunInBackground && wasPressed;
        }
        ...
        private void OnScrollCallback(InputAction.CallbackContext context)
        {
            ...
            state.scrollDelta = (scrollDelta / InputSettings.scrollWheelDeltaPerTick) * scrollDeltaPerTick;
        ...
        protected void FilterPointerStatesByType()
        {
            ...
            {
                ref var state = ref GetPointerStateForIndex(i);
                state.iaData.ReadDeviceState();
                state.CopyTouchOrPenStateFrom(state.iaData);
        ...
        public override int ConvertUIToolkitPointerId(IAPointerData sourcePointerData)
        {
            ...
            return sourcePointerData is ExtendedIAPointerData ep
        ...
        [SerializeField] protected InputActionAsset m_ActionsAsset;
        [SerializeField] protected InputActionReference m_PointAction;
        [SerializeField] protected InputActionReference m_MoveAction;
        [SerializeField] protected InputActionReference m_SubmitAction;
        [SerializeField] protected InputActionReference m_CancelAction;
        [SerializeField] protected InputActionReference m_LeftClickAction;
        [SerializeField] protected InputActionReference m_MiddleClickAction;
        [SerializeField] protected InputActionReference m_RightClickAction;
        [SerializeField] protected InputActionReference m_ScrollWheelAction;
        [SerializeField] protected InputActionReference m_TrackedDevicePositionAction;
        [SerializeField] protected InputActionReference m_TrackedDeviceOrientationAction;

        [SerializeField] protected bool m_DeselectOnBackgroundClick = true;
        [SerializeField] protected UIPointerBehavior m_PointerBehavior = UIPointerBehavior.SingleMouseOrPenButMultiTouchAndTrack;
        [SerializeField] internal CursorLockBehavior m_CursorLockBehavior = CursorLockBehavior.OutsideScreen;
        ...
        [NonSerialized] protected bool m_ActionsHooked;
        [NonSerialized] protected bool m_NeedToPurgeStalePointers;

        protected Action<InputAction.CallbackContext> m_OnPointDelegate;
        protected Action<InputAction.CallbackContext> m_OnMoveDelegate;
        protected Action<InputAction.CallbackContext> m_OnLeftClickDelegate;
        protected Action<InputAction.CallbackContext> m_OnRightClickDelegate;
        protected Action<InputAction.CallbackContext> m_OnMiddleClickDelegate;
        protected Action<InputAction.CallbackContext> m_OnScrollWheelDelegate;
        protected Action<InputAction.CallbackContext> m_OnTrackedDevicePositionDelegate;
        protected Action<InputAction.CallbackContext> m_OnTrackedDeviceOrientationDelegate;
        protected Action<object, InputActionChange> m_OnControlsChangedDelegate;

        // Pointer-type input (also tracking-type).
        [NonSerialized] protected int m_CurrentPointerId = -1; // Keeping track of the current pointer avoids searches in most cases.
        [NonSerialized] protected int m_CurrentPointerIndex = -1;
        ...
        internal InlinedArray<IAPointerModel> m_PointerStates;

        // Navigation-type input.
        protected IANavigationModel m_NavigationState;
