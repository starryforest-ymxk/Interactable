...
namespace Interactable.InputSystemSupport
{
    ...
    public class TrackedDeviceRaycaster : IABaseRaycaster
    {
        private struct IARaycastHitData
        {
            public IARaycastHitData(Graphic graphic, Vector3 worldHitPosition, Vector2 screenPosition, float distance)
            {
        ...
        public override void Raycast(IAPointerData eventData, List<IARaycastResult> resultAppendList)
        {
            if (eventData is ExtendedIAPointerData trackedEventData && trackedEventData.PointerType == UIPointerType.Tracked)
                PerformRaycast(trackedEventData, resultAppendList);
        }

        // Cached instances for raycasts hits to minimize GC.
        [NonSerialized] private List<IARaycastHitData> m_IARaycastResultsCache = new List<IARaycastHitData>();

        internal void PerformRaycast(ExtendedIAPointerData eventData, List<IARaycastResult> resultAppendList)
        {
            if (canvas == null)
                return;
            ...
            m_IARaycastResultsCache.Clear();
            SortedRaycastGraphics(canvas, ray, m_IARaycastResultsCache);

            // Now that we have a list of sorted hits, process any extra settings and filters.
            for (var i = 0; i < m_IARaycastResultsCache.Count; i++)
            {
                var validHit = true;

                var hitData = m_IARaycastResultsCache[i];
                ...
                if (validHit)
                {
                    var castResult = new IARaycastResult
                    {
                        gameObject = go,
                        raycaster = this,
        ...
        private static readonly List<IARaycastHitData> s_SortedGraphics = new List<IARaycastHitData>();
        private void SortedRaycastGraphics(Canvas canvas, Ray ray, List<IARaycastHitData> results)
        {
            ...
            for (var i = 0; i < graphics.Count; ++i)
            {
                ...
                {
                    Vector2 screenPos = RayCamera.WorldToScreenPoint(worldPos);
                    // mask/image intersection - See Unity docs on eventAlphaThreshold for when this does anything
                    if (graphic.Raycast(screenPos, RayCamera))
                    {
                        s_SortedGraphics.Add(new IARaycastHitData(graphic, worldPos, screenPos, distance));
                    }
                }
            }
